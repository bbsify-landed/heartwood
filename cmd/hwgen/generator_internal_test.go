...
func TestWriteTemplate_Errors(t *testing.T) {
	...
	// Execute error (not easy with simple templateData, but we can try with missing field if template uses it incorrectly)
	// Actually, template.Execute rarely fails if the data matches the structure.
	// Let's focus on formatting error by providing content that isn't valid Go.
